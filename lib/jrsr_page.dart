...
class JrSrGroupsPage extends StatefulWidget {
  const JrSrGroupsPage({super.key});

  @override
  State<JrSrGroupsPage> createState() => _JrSrGroupsPageState();
}

class _JrSrGroupsPageState extends State<JrSrGroupsPage> {
  ...
  Future<void> _fetchData() async {
    try {
      final snapshot = await FirebaseFirestore.instance
          ...
          .where('tags', arrayContains: 'Jr.-Sr. Interaction')
          .orderBy('startTime')
          .get();
      ...
      filtered.sort((a, b) {
        ...
        final bEnd = (bData['endTime'] as Timestamp).toDate();

        int getStatusPriority(DateTime start, DateTime end) {
          if (end.isBefore(now)) return 3; // Red
          if (start.isAfter(now)) return 2; // Yellow
          return 1; // Green
        }

        return getStatusPriority(aStart, aEnd).compareTo(getStatusPriority(bStart, bEnd));
      });
      ...
      print("\u{1F525} ERROR in fetching JR-SR groups: $e");
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: const Text("JR-SR Interaction Groups")),
      body: isLoading
                  ...
                  child: ListView(
                    children: [
                      const Padding(
                        ...
                        child: Text("\ud83d\udccc JR-SR Interaction Groups",
                            style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                      ),
                      ...filteredGroups.map(_buildGroupCard).toList(),
                    ],
                  ),
                ),
              ],
