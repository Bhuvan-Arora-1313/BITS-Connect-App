...
class EditGroupDetailsPage extends StatefulWidget {
  ...
  const EditGroupDetailsPage({required this.groupId});

  @override
  _EditGroupDetailsPageState createState() => _EditGroupDetailsPageState();
}
...
class _EditGroupDetailsPageState extends State<EditGroupDetailsPage> {
  ...
  final List<String> _allCourses = [
    ...
    "SOFT SKILLS FOR PROFESSI.. – BITS F226",
    ...
    "SYSTEMS ENGG PRINCIPLES – BITS F241",
    "TECH IN SOCIAL RESEARCH – GS F331",
    "TECHNICAL REPORT WRITING – BITS F112",
    "THEORY OF PLATES & SHELL – CE G553",
    ...
    "THESIS – BITS F423T",
    "THESIS – BITS F424T",
    "TIME SERIES ANALYSIS & F.. – MPBA G512",
    "TRANSPORT PHENOMENA – CHE F414",
    "URBAN HYDROLOGY – CE F422",
    "VLSI ARCHITECTURE – MEL G642",
    "VIBRATIONS AND CONTROL – ME F319",
    "WATER & WASTEWATER TREAT – CE F342",
    "WATER AND WASTEWATER TRE.. – EE G502",
  ...
  void _loadGroupData() async {
    ...
    if (data != null) {
      setState(() {
        ...
        _isPermanent = data['isPermanent'] ?? false;
        _selectedCourses = List<String>.from(data['courses'] ?? []);
        _selectedTags = List<String>.from(data['tags'] ?? []);
        _locationController.text = data['location'] ?? '';
        _maxMembersController.text = (data['maxMembers'] ?? '').toString();
        ...
        if (data['startTime'] != null && data['endTime'] != null) {
          ...
          _isToday = start.year == now.year && start.month == now.month && start.day == now.day;
        }
      });
    }
  }
  ...
  Future<void> _pickDuration() async {
    final durationMinutes = await showDialog<int>(
      ...
      builder: (context) => SimpleDialog(
        ...
        children: [
          for (int min in [30, 60, 90, 120])
            SimpleDialogOption(
              onPressed: () => Navigator.pop(context, min),
              child: Text("${min ~/ 60}h ${min % 60}m"),
            )
        ],
      ),
    );
    ...
    if (durationMinutes != null) {
      setState(() => _duration = Duration(minutes: durationMinutes));
    }
  }

  Future<void> _updateGroupDetails() async {
    if (!_formKey.currentState!.validate() || _startTime == null || _duration == null) return;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text("Edit Group Details")),
      body: Padding(
              ...
              Row(
                children: [
                  ...
                  if (!_isToday)
                    TextButton(
                      onPressed: _pickDate,
                      child: Text(_selectedDate != null ? DateFormat.yMd().format(_selectedDate!) : "Pick Date"),
                    )
                ],
              ),
              ListTile(
  ...
  Widget _buildCourseSelector() {
    return MultiSelectDialogField(
      items: _allCourses.map((c) => MultiSelectItem<String>(c, c)).toList(),
